...
public:
    LineTrackerProcessor() : Node("line_tracker_node"), mode_(false), k_(0.15), base_vel_(100) {
        ...
        vel_pub_ = this->create_publisher<geometry_msgs::msg::Vector3>("topic_dxlpub", qos_profile);
...
private:
    ...
    cv::Mat drawResult(cv::Mat &bin_roi, cv::Mat &stats, int best_idx) {
        ...
        cv::circle(display, cv::Point((int)last_line_x_, (int)last_line_y_), 3, cv::Scalar(0, 0, 255), -1);
        return display;
    }
